...
async def get_products(
    ...
    for item in items :
        ...
        pr = db.query(cart_model.Product).filter(item.productID==cart_model.Product.id).first()
        ...
        seller_user = db.query(seller.Seller).filter(pr.sellerID==seller.Seller.id).first()
    ...
    cur_order_items = [schema.OrderItem.model_validate(item) for item in cur_order_items]
    
    cur_order =schema.OrderCheckOut.model_validate({"totalCost":cur_order.totalCost , "order_item":cur_order_items})
    return cur_order
